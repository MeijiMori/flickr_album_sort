...
def oauth_requests():
    ...
    oauth_verifier = input("[PIN Code]> ")  # 上記PINコードを入力する
...
if __name__ == '__main__':
    ...
    parser.add_argument("sort_key", type=str, help="What to sort by album's [ id, title, update_date,"
    ...
    for album_id in my_albums.album_list:
        ...
        if sort_key == "id":
            ...
            # No case
            #id,title,update_date,create_date,views,tag
            print("\n Please input sort key > \
                    {0} or {1} or {2} or {3} or {4}".format("id", "title", \
                    "update_date", "create_date", "views_count", "tag"))
            exit()
